...
run_uni <- function(X, Y, version, IID = TRUE) {
  compute_akc_variance_auto(X, Y, IID = IID, version = version)
}

run_mv <- function(X, Y, version, IID = TRUE) {
  compute_akc_multivariate_variance_auto(X, Y, IID = IID, version = version)
}
...
run_benchmarks <- function(reps = 5) {
  ...
  for (sc in scenarios) {
    ...
    selected <- select_kernel_version(Y, X)
...
run_acor_benchmarks <- function(reps = 5) {
  ...
  for (sc in scenarios) {
    ...
    akc_ver <- select_kernel_version(Y, X)
