...
Sigma_akc_v1_wrapper <- function(X, Y) {
  ...
  var <- Sigma_akc_v1(X, Y, tau_XY, tau_Y, p_Y)
...
Sigma_akc_v2_wrapper <- function(X, Y) {
  ...
  var <- Sigma_akc_v2(X, Y, tau_XY, tau_Y, p_Y)
  list(akc = akc, var = var)
}
...
test_that("K_p_vec_v1 and K_p_vec_v2 produce identical results to original", {
  ...
  for (tc in test_cases) {
    ...
    K_p_v1 <- K_p_vec_v1(Y, tau_Y)
    K_p_v2 <- K_p_vec_v2(Y, tau_Y)
...
test_that("H_bar_vec_v1 and H_bar_vec_v2 produce identical results to original", {
  ...
  for (tc in test_cases) {
    ...
    H_v1 <- H_bar_vec_v1(X, Y)
    H_v2 <- H_bar_vec_v2(X, Y)
    ...
    expect_equal(H_v2, H_orig, tolerance = 1e-10,
                 info = sprintf("%s: H_bar_vec_v2 should match original", tc$name))
  }
})
...
test_that("K_tau_vec_v1 and K_tau_vec_v2 produce identical results to original", {
  ...
  for (tc in test_cases) {
    ...
    K_tau_v1 <- K_tau_vec_v1(X, Y, tau_XY)
    K_tau_v2 <- K_tau_vec_v2(X, Y, tau_XY)
...
test_that("Sigma_akc_multivariate_v1 and _v2 produce identical results to original", {
  ...
  for (tc in test_cases) {
    ...
    res_v1 <- Sigma_akc_multivariate_v1(X, Y)
    res_v2 <- Sigma_akc_multivariate_v2(X, Y)
...
test_that("Runtime benchmark: K_tau - Original vs V1 vs V2", {
  ...
  for (n in sample_sizes) {
    ...
    time_v1 <- time_execution(quote(K_tau_vec_v1(X, Y, tau_XY)),
                              replications = n_replications)$median
    time_v2 <- time_execution(quote(K_tau_vec_v2(X, Y, tau_XY)),
                              replications = n_replications)$median
...
test_that("Runtime benchmark: Sigma_akc_multivariate - Original vs V1 vs V2", {
  ...
  for (n in sample_sizes) {
    ...
    time_orig <- time_execution(quote(Sigma_akc_multivariate(X, Y)),
                                replications = n_replications)$median
    time_v1 <- time_execution(quote(Sigma_akc_multivariate_v1(X, Y)),
                              replications = n_replications)$median
    time_v2 <- time_execution(quote(Sigma_akc_multivariate_v2(X, Y)),
                              replications = n_replications)$median
