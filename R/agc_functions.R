...
agc_assemble_hac_variance <- function(rho, zeta_3Y, sigma_zeta,
  ...
  factor_val <- 1 / ((1 - zeta_3Y)^2)
  factor_val * (A_spear + (2 * rho * B_both) / (1 - zeta_3Y) +
...
agc_assemble_hac_covariance <- function(rho_j, rho_i, zeta_3Y, sigma_zeta,
  ...
  factor_val <- 1 / ((1 - zeta_3Y)^2)
  factor_val * (A2_spear + (rho_j * B_one) / (1 - zeta_3Y) +
...
# ============================================================================
# KERNEL: V2 Fenwick tree -- O(n log n)
# ============================================================================
# Fenwick functions are defined in akc_functions.R (shared via package namespace).
# If running standalone, they must be available.
...
kernel_agc_v2 <- function(x_rank, y_rank, rho) {
  ...
  # --- Pass 1: c_both_less and c_xless_yeq via Fenwick tree ---
  ...
  # --- Pass 2: c_xeq_yless and c_both_eq within X groups ---
  c_xeq_yless <- integer(N)
  c_both_eq <- integer(N)
  
  for (group_name in names(X_groups)) {
    indices <- X_groups[[group_name]]
    y_ranks_in_group <- Y_compressed[indices]
    ...
    for (kk in seq_along(sorted_indices)) {
      i <- sorted_indices[kk]
      yr <- sorted_y_ranks[kk]
      if (yr == prev_y_rank) {
    ...
    xy_pairs <- paste(X_compressed[indices], Y_compressed[indices], sep = "_")
    pair_counts <- table(xy_pairs)
    for (i in indices) {
      pair_key <- paste(X_compressed[i], Y_compressed[i], sep = "_")
      c_both_eq[i] <- pair_counts[[pair_key]]
    }
  }
  
  # H_bar(x_i, y_i) for all i
  H_bar_ii <- (c_both_less + 0.5 * c_xeq_yless +
                 0.5 * c_xless_yeq + 0.25 * c_both_eq) / N
  
  # --- Compute g_2[i] = (1/N) * sum_k H_bar(x_i, y_k) ---
  y_tab <- table(factor(y_rank, levels = Y_unique))
  n_y <- as.numeric(y_tab)
  ...
  n_x_lt <- cum_n_x[X_compressed]
  
  w_full <- n_y_gt + 0.5 * n_y_eq
  w_half <- 0.5 * n_y_gt + 0.25 * n_y_eq
  
  # Group sums for x_j == x_i term
  ...
  # --- Compute g_1[i] = (1/N) * sum_{i'} H_bar(x_{i'}, y_group(i)) ---
  n_x_gt <- N - n_x_lt - n_x_eq
  
  T_m <- numeric(M)
  for (m in 1:M) {
    mask_lt <- Y_compressed < m
    mask_eq <- Y_compressed == m
